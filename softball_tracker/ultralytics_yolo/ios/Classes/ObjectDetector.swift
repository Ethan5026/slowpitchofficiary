  ...
  private func processObservations(for request: VNRequest, error: Error?) {
    DispatchQueue.main.async {
      if let results = request.results as? [VNRecognizedObjectObservation] {
        ...
        // Change the ratio from 4:3 to 16:9, the camera being accessed has a 16:9 ratio, not 4:3
        let ratio: CGFloat = (height / width) / (16.0 / 9.0)  // .photo
        ...
        for i in 0..<100 {
          if i < results.count && i < self.numItemsThreshold {
            ...
            print("🔍 Full label info: \(prediction.labels)")
            recognitions.append([
              "label": label,
